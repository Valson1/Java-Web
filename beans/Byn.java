...
public final class Byn implements Comparable<Byn> {
	...
	if (value < 0) {
	    throw new NegativeArgumentException(EXCEPTION_MESSAGE_BYN_VALUE + value, value);
	}
	this.value = value;
    }
	...
	if (rubs < 0) {
	    throw new NonpositiveArgumentException(EXCEPTION_MESSAGE_BYN_RUBS + rubs);
	}
	if (coins < 0 || coins >= VALUE_DIVISOR) {
	    throw new NonpositiveArgumentException(EXCEPTION_MESSAGE_BYN_COPS + coins);
	}
